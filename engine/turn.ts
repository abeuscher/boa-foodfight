...
export const runScenario = (
  ...
  //
  // L2 (escort), L6 (eradicate), L8 (recruit-count): the score path is
  // a capture-objective construct (POST ownership + web-hold progress)
  // and does not model "did the escortee get through" / "are all
  // spiders dead" / "were enough cockroaches recruited". Reaching the
  // turn cap on any of these is an unambiguous ant loss (spider win)
  // with no score tiebreak — escort per roadmap §4.3.1, eradicate per
  // §4.3.2, recruit-count per §4.3.3 (too few recruited in time). The
  // decision keys off the scenario `victoryCondition.kind`; a state
  // without the field defaults to capture-post, so the L1 path is
  // byte-identical.
