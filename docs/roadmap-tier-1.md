...
### 7.17 Start Screen (title / main menu)

Cross-track change request (UX→Dev, exchange #13; full prose in
`docs/change-request-protocol.md` §5), disposition **ratified with one
route correction**. Doc-only; **zero engine work** — the substantive
forward pieces are all client-side.

`docs/ui-start-screen-spec.md` (RATIFIED) defines the application entry
point: a four-item menu — **Continue · New Game · Load Game · Options**
— with keyboard/pointer selection reusing main-screen conventions, **no
Quit/Exit** (browser client), and visual treatment §D-deferred (cube
memo). **New Game is the only item wired live** this pass; the other
three are present-but-stubbed (render, hold their place, no-op until
wired).

Dev-verified: the **fresh-campaign initial state already exists and is
cheap** — `INITIAL_STATE` in `client/src/fixture.ts` (generated by
`pnpm gen:fixture` from the frozen `data/level-1/` build); New Game is
`setState(INITIAL_STATE)`, the loop's defeat→reset one-liner. **Route
correction:** the Briefing view is RECORDED but not built, and today's
L1 entry routes _through_ the Hill — so New Game's final
`→ Briefing → cube` destination is blocked on building the Briefing.
**Interim route A (UX call):** New Game routes straight into the live L1
scenario (skipping both Briefing and Hill, honoring the skip-Hill
intent), converging to `New Game → Briefing → scenario` with no menu
change once the Briefing is built (the recommended next build). **Skip-Hill
ruling:** a new player gets no pre-L1 army management before the first
fight — intended for the L1 tutorial context. **Recorded forward
surprises:** Continue + Load Game are blocked on a client campaign-save
layer that does not exist (engine `world-save.ts` primitive present, but
no browser localStorage/save-list/resume — distinct from the §7.13
mid-scenario replay save); Options has no client settings system yet.
Ratifying supersedes the current Start stub (Start→Hill).
