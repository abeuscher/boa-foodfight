...
### Exchange #13 — Start Screen (title / main menu)

**Request (UX):** ratify `ui-start-screen-spec.md` — the application
entry point: a four-item menu (Continue · New Game · Load Game ·
Options), keyboard/pointer selection reusing main-screen conventions,
no Quit/Exit (browser client), visual treatment §D-deferred. **New Game
is the only item wired live** this pass; the other three are
present-but-stubbed (forward surfaces). Two New-Game confirmations
asked (does a fresh-campaign initial state exist/is it cheap; does the
`New Game → Briefing → cube` route exist in the current build), plus a
request to flag any forward-wiring surprises for Continue/Load/Options.

**Reply (Dev) — Disposition: Ratify, with one route correction.**

- **Confirm 1 (fresh-campaign init): EXISTS, cheap.** `INITIAL_STATE`
  in `client/src/fixture.ts` (generated by `pnpm gen:fixture` from the
  frozen `data/level-1/` build) is a complete authored start
  (`scenarioIndex: 0`, `gold: 120`, 28-unit roster, `rngSeed`). New
  Game is `setState(INITIAL_STATE)` — the loop's defeat→reset one-liner.
  Zero new authoring/engine work; config-derived as recorded.
- **Confirm 2 (route): CORRECTED.** The **Briefing view is not built**
  (`ui-briefing-spec.md` RECORDED, no client component); and today's
  real L1 entry routes **through** the Hill (Start-stub → Hill → Deploy
  → scenario), not past it. So `New Game → Briefing → cube (skip Hill)`
  is feasible but blocked on building the Briefing, and skip-Hill is a
  behavior change from what's wired.
- **Route decision (UX): A (interim).** Until the Briefing exists, New
  Game routes **straight into the live L1 scenario** — skipping both
  Briefing and Hill, honoring the ratified skip-Hill intent. Converges
  to `New Game → Briefing → scenario` with **no menu change** once the
  Briefing is built (dev's recommended next build — the actual missing
  surface). Route B (New Game → Hill) declined: it contradicts the
  spec's skip-Hill line.
- **Skip-Hill ruling (recorded, conscious):** a brand-new player gets
  **no pre-L1 army management** (Organize/Recruit/Shop) before the first
  fight; intended for the L1 tutorial context, not a side effect.
- **Forward-wiring surprises (recorded in spec §Forward dependencies):**
  Continue + Load Game are **blocked on a client campaign-save layer
  that does not exist** — the engine has the serialize primitive
  (`world-save.ts`) and the harness persists to disk, but the browser
  client has no localStorage/save-list/resume (`System.tsx`: format
  "awaits a UI"). Distinct from the §7.13 mid-scenario save
  (`scenario-save.ts`, replay). Options has **no client settings system**
  at all; stub no-op correct, scaffolding trivial when wired.
- **Supersession:** ratifying replaces the current Start stub
  (Start→Hill) with the four-item menu; straight replacement, no
  conflict. Dev builds the menu + route A on go-ahead.
- **Decision record:** roadmap §7.17.

**Outcome:** Resolved + ratified. Spec RATIFIED and promoted
`docs/drafts/` → `docs/ui-start-screen-spec.md`, with the dev-confirmed
init fact, interim route A, the skip-Hill ruling, and the
client-save-layer / settings forward surprises folded in. The
`exchange-13-start-screen.md` routing doc is marked RESOLVED (this §5
entry is canonical). Zero engine work; the substantive forward pieces
(client save layer, Briefing view, settings system) are all
client-side. Recorded `roadmap-tier-1.md` §7.17.
